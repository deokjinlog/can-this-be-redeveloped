...
def 일주택(v=True, span="세대 전원 1주택"):
    return Fact(v, Grade.P1, "등기부·세대별 주민등록표", span)

def 미도래(단계="조합설립인가"):
    """진행단계상 아직 오지 않은 사건 — '미확인' 이 아니라 '확정적으로 없음'."""
    return Fact(None, Grade.S1, "정보몽땅 진행단계", f"현재 '{단계}' — 해당 단계 미도래")
...
cases.append(("①소유15·거주3", "승계불가(현금청산 대상)", Case(
    ...
    착공일=미도래(),          # §37③3호(재개발도 해당)는 착공이 있어야 성립
    일세대일주택=일주택(),
...
cases.append(("②소유12·합산거주5·1주택", "승계가능", Case(
    사업유형="재개발", 기준일=D, 기준일_기준="등기일", 투기과열지구=True,
    일세대일주택=일주택(),
...
# ②-b 기간은 채웠는데 다주택 → §39②4호는 '1세대 1주택자로서' 라 불성립
cases.append(("②b기간충족·다주택", "승계불가(현금청산 대상)", Case(
    사업유형="재개발", 기준일=D, 기준일_기준="등기일", 투기과열지구=True,
    착공일=미도래(),          # §37③3호(재개발도 해당)는 착공이 있어야 성립
    일세대일주택=일주택(False, "세대원 명의 주택 2건"),
    취득일=deungi(date(2014, 9, 1), "갑구 소유권이전 접수 2014년9월1일"),
    거주개시일=chobon(date(2021, 6, 1), "합산 5.3년"),
)))

# ②-c 1주택 여부 미확인 → 기간을 채워도 확정하지 않는다(반올림 금지)
cases.append(("②c1주택미확인", "확인필요", Case(
    사업유형="재개발", 기준일=D, 기준일_기준="등기일", 투기과열지구=True,
    취득일=deungi(date(2014, 9, 1), "갑구 소유권이전 접수 2014년9월1일"),
    거주개시일=chobon(date(2021, 6, 1), "합산 5.3년"),
)))
...
# ⑦ 상속·이혼 취득은 §39② 본문상 '양수' 가 아니다 → 제한 자체가 없음
cases.append(("⑦상속취득", "승계가능", Case(
    사업유형="재건축", 기준일=D, 기준일_기준="등기일", 투기과열지구=True,
    취득사유_상속이혼=Fact(True, Grade.P1, "가족관계증명·등기부",
                    "갑구 소유권이전 원인 '상속'"),
)))

# ⑧ 매매 취득이면 그 게이트는 열리지 않는다(오적용 회귀)
cases.append(("⑧매매취득", "승계불가(현금청산 대상)", Case(
    사업유형="재개발", 기준일=D, 기준일_기준="등기일", 투기과열지구=True,
    취득사유_상속이혼=Fact(False, Grade.P1, "등기부", "원인 '매매'"),
    일세대일주택=일주택(False, "다주택"),
    착공일=미도래(),
    취득일=deungi(date(2014, 9, 1), "접수 2014-09-01"),
    거주개시일=chobon(date(2021, 6, 1), "합산 5.3년"),
)))
