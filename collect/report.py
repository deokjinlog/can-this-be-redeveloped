...
def build() -> str:
    ...
    L += ["", "## 해제 이력 — 구청 고시에는 없다", "",
          ...
          "",
          "→ 그래서 서울시 쪽을 찾아봤는데, 여기서도 못 찾았다:", "",
          "| 찾아본 곳 | 결과 |", "|---|---|",
          "| 관악구 고시공고 (CMS·eminwon 전 기간) | 정비구역 해제 0건 |",
          "| 서울시 고시공고 `www.seoul.go.kr/news/news_notice.do` | '정비구역' 자체가 6건 — 주 창구가 아님 |",
          "| 서울도시공간포털 `urban.seoul.go.kr` | 결정고시 조회 메뉴는 있으나 지도 기반이라 API 가 JS 안에 |",
          "",
          "여기서부터는 브라우저 네트워크 로그가 필요하다(도시계획조회 결정고시 화면이",
          "어떤 요청으로 목록을 받는지). 그때까지 해제 이력은 지금처럼 **고시도형의 세대 구분**",
          "(도형 겹침 + 고시일자, 서울 78건)으로 근사한다 — 확증이 아니라 근사임을 표시한 채로.", ""]
